...
export default function ProductCard({ product }) {
  ...
  const imageSrc = product.images?.[0] || 'https://placehold.co/800x800?text=Product';
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col px-5 pb-5 pt-4">
        ...
        <p className="mt-3 flex-1 text-base leading-7 text-palette-dark/70 line-clamp-3">{description}</p>
        <div className="triangle relative ml-auto mt-6 rounded-tl-sm bg-palette-lighter px-5 pb-2 pt-3 text-right">
          <span className="text-sm uppercase tracking-[0.14em] text-palette-dark/55">Price</span>
          <p className="mt-1 text-xl font-semibold text-palette-dark">{price}</p>
        </div>
      </div>
    </Link>
  );
